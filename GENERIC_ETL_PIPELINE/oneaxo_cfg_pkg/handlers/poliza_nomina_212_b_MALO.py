...
#   - Cada documento renumera sus detalles desde 1 en campo de 3 posiciones, sin ceros; el número queda a la izquierda y se completa con espacios a la derecha.
# ============================================================================
...
class PolizaNomina212BHandler:
    ...
    def build_canonical(
        ...
        **kwargs,
        ...
        for group, group_details in groups.items():
            ...
            original_idext = self._get_group_original_idext(
                group=group,
                group_details=group_details,
                fallback_idext=header_line[1:13].strip(),
            )
    ...
    def _get_group_original_idext(
        ...
        """
        Obtiene el IDEXT base que debe usar cada grupo.

        Regla del layout 212_B:
            BASECO -> M003NM10N2A6
            TOMMY  -> M003NM10T2T6

        Nota importante:
        En algunos archivos mixtos el header y el Assignmentnumber vienen con
        el IDEXT BASECO para todos los registros, incluso para cost centers MX003
        de TOMMY. Por eso no basta con copiar el Assignmentnumber; se normaliza
        el IDEXT de acuerdo con el grupo detectado.
        """
        candidate_idext = ""

        for line in group_details:
            assignment_idext = line[41:59].strip()

            if len(assignment_idext) >= 12:
                candidate_idext = assignment_idext[:12]
                break

        if not candidate_idext and fallback_idext:
            candidate_idext = fallback_idext[:12]

        if not candidate_idext:
            raise ValueError(f"No se pudo determinar IDEXT base para grupo {group}")

        return self._normalize_idext_for_group(
            idext=candidate_idext,
            group=group,
        )

    def _normalize_idext_for_group(self, idext: str, group: str) -> str:
        """
        Normaliza el IDEXT según el grupo.

        Posiciones del IDEXT:
            M003NM10N2A6
                    ^ ^
                    | |
                    | +-- letra de documento inicial
                    +---- marca de compañía

        BASECO debe iniciar como N?A6.
        TOMMY debe iniciar como T?T6.
        Después, _increment_idext() incrementa la penúltima letra:
            BASECO: A6, B6, C6...
            TOMMY : T6, U6, V6...
        """
        fixed = list(idext.ljust(12)[:12])

        if len(fixed) < 12:
            raise ValueError(f"IDEXT inválido para grupo {group}: {idext}")

        if group == GROUP_TOMMY:
            fixed[8] = "T"
            fixed[10] = "T"
        elif group == GROUP_BASECO:
            fixed[8] = "N"
            fixed[10] = "A"
        else:
            raise ValueError(f"Grupo no soportado para IDEXT: {group}")
    ...
    def _replace_idext(self, line: str, new_idext: str) -> str:
        """
        Reemplaza el IDEXT principal en posiciones [1:13].

        En detalles también reemplaza Assignmentnumber [41:59], porque ese campo
        debe coincidir con el IDEXT del documento generado.
        """
        fixed_idext = new_idext.ljust(12)[:12]
        updated = line[:1] + fixed_idext + line[13:]

        if updated.startswith("D") and len(updated) >= 59:
            updated = updated[:41] + new_idext.ljust(18)[:18] + updated[59:]
    ...
    def _renumber_detail(self, detail_line: str, number: int, new_idext: str) -> str:
        ...
        return line[:13] + str(number).ljust(3)[:3] + line[16:]
    ...
    def _build_neto_detail_line(
        ...
        line = line[:13] + str(number).ljust(3)[:3] + line[16:]
